...
function goToChildResult(childId) {
    ...
    if (childHistory.length > 0) {
        ...
        window.location.href = 'mypage/mypage-result.html?childId=' + encodeURIComponent(child.id || '');
        return;
    }
...
function hasSavedResult() {
    return !!localStorage.getItem('nollpic_latest_result');
}

function goBottomResult() {
    if (!hasSavedResult()) {
        alert('아직 저장된 검사 결과가 없어요. 먼저 5가지 검사를 완료해주세요.');
        return;
    }
    window.location.href = 'mypage/mypage-result.html';
}

function goBottomMypage() {
    nextPage(3);
}
